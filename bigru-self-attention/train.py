...
# -*- coding: utf-8 -*-
"""
训练网络，并保存模型，其中GRU的实现采用Python中的keras库
"""
import json
...
from tensorflow.python.keras.layers import Embedding, GRU, Dropout, Dense, Input, Activation, Bidirectional
...
def train_gru(n_symbols, embedding_weights, x_train, y_train, x_test, y_test):
    ...
    # GRU层并返回序列和状态
    # gru_layer, hidden_state = GRU(units=50, activation='tanh', return_sequences=True, return_state=True)(embedding_layer)
    gru_layer = Bidirectional(GRU(units=50, activation='tanh', return_sequences=True))(embedding_layer)
    # 注意力层
    attention_layer = SelfAttentionLayer()(gru_layer)
    # # Dropout层
    ...
    print("Save model...")
    save_path = '../model/bigru-self-attention.h5'
    model.save(save_path)  # 保存模型
    # 转换为JSON格式
    json_string = model.to_json()
    with open('../model/bigru-self-attention.json', 'w') as outfile:
