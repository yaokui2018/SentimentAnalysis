...
from tensorflow.python.keras.layers import Embedding, LSTM, Dropout, Dense, Input, Activation
...
def train_lstm(n_symbols, embedding_weights, x_train, y_train, x_test, y_test):
    ...
    # LSTM层并返回序列和状态
    lstm_layer, state_h, state_c = LSTM(units=50, activation='tanh', return_sequences=True, return_state=True)(
        embedding_layer)
    # 注意力层
    ...
    print("Save model...")
    save_path = '../model/lstm-attention.h5'
    model.save(save_path)  # 保存模型
    # 转换为JSON格式
    json_string = model.to_json()
    with open('../model/lstm-attention.json', 'w') as outfile:
